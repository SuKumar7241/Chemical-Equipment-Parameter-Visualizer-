#!/usr/bin/env python3
"""
Enhanced Desktop Application with all advanced features - Windows compatible
"""
...
class ChartWidget(QWidget):
    ...
    def plot_dataset_analysis(self, statistics, columns):
        ...
        if data_types:
            ...
            wedges, texts, autotexts = ax1.pie(data_types.values(), labels=data_types.keys(),
                                              autopct='%1.1f%%', colors=colors)
            ax1.set_title('Data Type Distribution', fontweight='bold')
        
        # Plot 2: Missing values
        ax2 = self.figure.add_subplot(2, 2, 2)
        col_names = [col['column_name'] for col in columns[:8]]  # Top 8 columns
        null_counts = [col.get('null_count', 0) for col in columns[:8]]
        
        if any(null_counts):
            bars = ax2.bar(range(len(col_names)), null_counts, color='lightcoral')
            ax2.set_title('Missing Values by Column', fontweight='bold')
            ax2.set_xlabel('Columns')
            ax2.set_ylabel('Null Count')
            ax2.set_xticks(range(len(col_names)))
            ax2.set_xticklabels(col_names, rotation=45, ha='right')
        ...
        # Plot 3: Dataset summary statistics
        ax3 = self.figure.add_subplot(2, 2, 3)
        stats_data = statistics.get('statistics_data', {})
        
        # Get numeric columns and their means
        numeric_cols = []
        numeric_means = []
        for col_name, col_stats in stats_data.items():
            if isinstance(col_stats, dict) and 'mean' in col_stats:
                numeric_cols.append(col_name[:10])  # Truncate long names
                numeric_means.append(col_stats['mean'])
        
        if numeric_cols:
            bars = ax3.bar(range(len(numeric_cols)), numeric_means, color='skyblue')
            ax3.set_title('Numeric Column Means', fontweight='bold')
        ...
        # Plot 4: Summary overview
        ax4 = self.figure.add_subplot(2, 2, 4)
        summary_labels = ['Total Records', 'Numeric Cols', 'Categorical Cols', 'Missing Values']
        summary_values = [
        ...
        bars = ax4.bar(range(len(summary_labels)), summary_values, color='lightgreen')
        ax4.set_title('Dataset Summary', fontweight='bold')
        ax4.set_xticks(range(len(summary_labels)))
        ax4.set_xticklabels(summary_labels, rotation=45, ha='right')
...
class EnhancedDatasetAnalyzer(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        self.setWindowTitle("Dataset Analyzer - Enhanced Version")
    ...
    def create_login_tab(self):
        ...
        subtitle = QLabel("Professional Desktop Application")
    ...
    def update_stats_cards(self, datasets):
        ...
        avg_columns = int(sum(d.get('total_columns', 0) for d in datasets) / max(total_datasets, 1))
...
def main():
    print("=== Dataset Analyzer - Enhanced Version ===")
    print("Starting complete desktop application...")
    print("Features: Authentication, Upload, Analysis, Charts, History")
    
    app = QApplication(sys.argv)
    app.setApplicationName("Dataset Analyzer - Enhanced")
    app.setStyle('Fusion')
    
    window = EnhancedDatasetAnalyzer()
    window.show()
    window.raise_()
